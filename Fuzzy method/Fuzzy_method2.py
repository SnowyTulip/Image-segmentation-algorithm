...
def U(g,k,g1,g2,C):
    ...
    '''
    u = g1[k] if g <= k else g2[k]
    C_val = max(C)
    res = (1) / (1 + abs(g - u) / C_val)
    return res
...
def Get_E(img_gray,g1,g2,C,l = 8):
    ...
    pixel = get_pixels(img_gray,l)
    #A three-tier python loop, just like a snail ....
    with tqdm(total=4**l,colour= "red") as pbar:
        pbar.set_description(f'Processing')
        for k in range(2**l):
            acc = 0
            for g in range(2**l):
                acc += S(U(g,k,g1,g2,C)) * pixel[g]
                pbar.update(1)
